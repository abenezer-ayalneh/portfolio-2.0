...
const SCRIPT: ScriptLine[] = [
	...
	{
		kind: 'tool',
		call: 'callModel({ provider: "anthropic" })',
		note: 'streaming',
	},
	{
		kind: 'answer',
...
function Cursor() {
	return <span className="cursor-blink ml-0.5 inline-block w-[0.55ch] bg-primary text-transparent">_</span>
}
...
function AgentTerminal() {
	...
	useEffect(() => {
		...
		const delay = line.kind === 'cmd' ? 700 : line.kind === 'thinking' ? 950 : line.kind === 'tool' ? 520 : 400
		...
		<div ref={ref} className="terminal-panel crt-flicker overflow-hidden rounded-xl">
			...
			<div className="flex items-center gap-2 border-b border-border bg-muted/40 px-4 py-2.5">
				...
				<span className="ml-auto flex items-center gap-1.5">
					...
					<span className="font-mono text-[0.65rem] uppercase tracking-wider text-primary">online</span>
				</span>
			</div>
					...
					if (line.kind === 'cmd') {
						return (
							<div key={i} className="text-foreground">
								<span className="text-primary">$</span> <span className="text-glow">{line.text}</span>
								{isCurrent && <Cursor />}
							</div>
						)
					}
					...
					if (line.kind === 'tool') {
						return (
							<div key={i} className="text-muted-foreground">
								<span className="text-primary/70">↳</span> <span className="text-foreground">{line.call}</span>
	...
	return (
		<section id="ai" aria-labelledby="ai-heading" className="relative overflow-hidden py-20 lg:py-28">
			...
			<div
				...
				style={{
					background: 'radial-gradient(ellipse 60% 50% at 80% 30%, hsl(var(--primary) / 0.1), transparent 70%)',
				}}
			/>
